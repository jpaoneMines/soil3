...
bool extensionExist( const char* ext ) {
    GLint n, i;
    glGetIntegerv(GL_NUM_EXTENSIONS, &n);
    for (i = 0; i < n; i++)
        if( strcmp( (char const*)glGetStringi(GL_EXTENSIONS, i), ext ) == 0 )
            return true;
    return false;
}
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if(
			//(NULL == strstr( (char const*)glGetStringi( GL_EXTENSIONS ),
				extensionExist("GL_ARB_texture_non_power_of_two" ) )
			//)
		{
...
int query_tex_rectangle_capability( void )
{
	...
	if( has_tex_rectangle_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if(
			//(NULL == strstr( (char const*)glGetStringi( GL_EXTENSIONS ),
				extensionExist("GL_ARB_texture_rectangle" ) //)
		&&
			//(NULL == strstr( (char const*)glGetStringi( GL_EXTENSIONS ),
				extensionExist("GL_EXT_texture_rectangle" ) //)
		&&
			//(NULL == strstr( (char const*)glGetStringi( GL_EXTENSIONS ),
				extensionExist("GL_NV_texture_rectangle" ) //)
			)
		{
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	if( has_cubemap_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if(
			//(NULL == strstr( (char const*)glGetStringi( GL_EXTENSIONS ),
				extensionExist("GL_ARB_texture_cube_map" ) //)
		&&
			//(NULL == strstr( (char const*)glGetStringi( GL_EXTENSIONS ),
				extensionExist("GL_EXT_texture_cube_map" ) //)
			)
		{
			/*	not there, flag the failure	*/
...
int query_DXT_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if( //NULL == strstr( (char const*)glGetStringi( GL_EXTENSIONS ),
				extensionExist("GL_EXT_texture_compression_s3tc" ) )
		{
			/*	not there, flag the failure	*/
